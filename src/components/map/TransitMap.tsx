...
import type { Coordinate, MockLocation } from '../../data/mockData';
...
const StationMarker = memo(function StationMarker({ stop, index, selected, routeStop, onSelect }: {
  stop: MockLocation; index: number; selected: boolean; routeStop: boolean; onSelect: (stop: MockLocation) => void;
}) {
...
export const TransitMap = memo(function TransitMap({ mapRef, initialRegion, canInteract, onReady, onLoaded, stations, routeStop, selectedStationId, shape, color, store, selectedBusId, onSelectBus, onSelectStation, userLocation }: {
  ...
  stations: MockLocation[]; routeStop: boolean; selectedStationId?: string; shape: Coordinate[]; color: string;
  store: LiveBusStore; selectedBusId?: string; onSelectBus: (bus: LiveBus) => void; onSelectStation: (station: MockLocation) => void;
